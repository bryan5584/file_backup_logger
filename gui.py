...
class BackupApp:
    def __init__(self, root):
        ...
        tk.Button(root, text="Parcourir", command=self.select_source).grid(row=0, column=2)
        ...
        tk.Button(root, text="Parcourir", command=self.select_dest).grid(row=1, column=2)

        # Compression checkbox
        self.compress_var = tk.BooleanVar()
        tk.Checkbutton(root, text="Compresser en ZIP", variable=self.compress_var).grid(row=2, column=1, sticky="w")

        # Backup button
        self.backup_button = tk.Button(root, text="Lancer la sauvegarde", command=self.run_backup)
    ...
    def backup_thread(self, source, destination, compress):
        try:
            ...
            msg = f"Sauvegarde réussie dans : {backup_path}\nFichiers copiés : {file_count}\nDurée : {duration:.2f} secondes"
